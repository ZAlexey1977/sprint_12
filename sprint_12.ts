// Task 01
// Создайте класс Goods, который имеет свойство title - string и _weight (private), number. Напишите конструктор,
// для задания данных свойств. Уточнения для всех задач далее. Если не упоминается про модификатор доступа то считаем что public.
...
class Goods {
  ...
  constructor(title: string, weight: number) {
    (this.title = title), (this._weight = weight);
  }
...
// Давайте исправим данную ситуацию. Для начала создадим класс Goods_03, в котором будут два свойства:
...
// В противном случае ставит 0. Также в сеттере реализуйте установку _adult в зависимости от возраста.
...
class User_07 {
  ...
  public a(age: number): number {
    return (this._age = age);
  }

  public bool(num: number): boolean {
...
// то напрямую изменить его в сеттере класса наследника будет нельзя. Примените обращение к сеттеру родителя через super.
...
// Task 11
// Видно, что когда мы переписали сеттер, то геттер не сработал. Хотя свойство сеттер - изменил.
// Т.е. если мы наследуемся от класса с гетером и сеттером, и переписываем сеттер, нужно переписать и геттер.
// Давайте в класс Test_10 допишем сеттер для _n. Если все сделать корректно, то console.log(t10.n);
// в проверке выше будет возвращать значение _n.

// Task 12
// Давайте попробуем сделать обратную операцию. Создайте класс Test_11, который наследуется от Test.
// Напишите в классе новый геттер, который возвращает _n. И запустите проверку.
// Цель - проверить будет ли работать сеттер, когда мы написали новый геттер в классе наследнике.
...
// Допишите руками в Test_11 сеттер, который изменяет _n следующим образом - если число меньше нуля - то ставим _n равное 0.
// Если больше то присваиваем значение. Если все сделано корректно, то проверки к заданию 12 заработают.
// А мы сделаем вывод - записали новый сеттер? Переписывайте геттер и наоборот, в классах наследниках.

// Task 14
// Создайте класс SignupForm с полями _email, _password, _passwordRepeat все поля protected.
// Создайте геттеры для свойств _email, _password. Создайте метод prepareString, private,
